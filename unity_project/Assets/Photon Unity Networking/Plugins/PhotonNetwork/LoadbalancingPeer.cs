				...
				public virtual bool OpCreateRoom (string roomName, RoomOptions roomOptions, TypedLobby lobby, Hashtable playerProperties, bool onGameServer)
				{
						...
						if (onGameServer) {
								if (playerProperties != null && playerProperties.Count > 0) {
										...
										op [ParameterCode.Broadcast] = true; //  : check if this also makes sense when creating a room?! // broadcast actor properties
								}
								...
								gameProperties [GameProperties.IsOpen] = roomOptions.isOpen; //  : check default value. dont send this then
								gameProperties [GameProperties.IsVisible] = roomOptions.isVisible; //  : check default value. dont send this then
