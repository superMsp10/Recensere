...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
		...
		internal protected Dictionary<int, PhotonView> photonViewList = new Dictionary<int, PhotonView> (); // : make private again
		...
		{
				//  : CAS must be implemented for OfflineMode
    ...
    #region Helpers
		...
		private void GameEnteredOnGameServer (OperationResponse operationResponse)
		{
				...
				if (!this.mCurrentGame.serverSideMasterClient)
    ...
    #region Implementation of IPhotonPeerListener
		...
		public void OnStatusChanged (StatusCode statusCode)
		{
				...
				case StatusCode.EncryptionFailedToEstablish:
						...
						this.OpAuthenticate (this.mAppId, this.mAppVersionPun, authV, this.CloudRegion.ToString (), requestLobbyStatistics);     //  : check if there are alternatives
						break;
		...
		internal Hashtable SendInstantiate (string prefabName, Vector3 position, Quaternion rotation, int group, int[] viewIDs, object[] data, bool isGlobalObject)
		{
				...
				// : reduce hashtable key usage by using a parameter array for the various values
		...
		public void OpRemoveCompleteCache ()
		{
				...
				this.OpRaiseEvent (0, null, true, options);
				//this.OpRaiseEvent(0, null, true, 0, EventCaching.RemoveFromRoomCache, ReceiverGroup.MasterClient);  //  : check who gets this event?
		}
		...
		public void SetLevelPrefix (short prefix)
		{
				...
				//  : should we really change the prefix for existing PVs?! better keep it!
				//foreach (PhotonView view in this.photonViewList.Values)
				//{
				//    view.prefix = prefix;
				//}
		...
		internal void RPC (PhotonView view, string methodName, PhotonPlayer player, bool encrypt, params object[] parameters)
		{
				...
				if (view.viewID < 1) {    // : check why 0 should be illegal
