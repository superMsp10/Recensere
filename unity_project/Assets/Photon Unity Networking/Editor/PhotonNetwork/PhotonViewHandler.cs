...
public class PhotonViewHandler : EditorWindow
{
		...
		internal static void HierarchyChange ()
		{
				...
				foreach (PhotonView view in pvInstances) {
						...
						view.prefix = -1;   //  : prefix could be settable via inspector per scene?!
		...
		//   fail if no ID was available anymore
		//   look up lower numbers if offset hits max?!
		...
		// : check if this can be internal protected (as source in editor AND as dll)
		public static void LoadAllScenesToFix ()
		{
				...
				foreach (string scene in scenes) {
						...
						PhotonViewHandler.HierarchyChange ();// : most likely on load also triggers a hierarchy change
